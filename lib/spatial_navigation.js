...
;(function($) {
  ...
  function focusElement(elem, sectionId, direction, leaveFor) {
    if (!elem) {
      return false;
    }
    ...
    var focusNscroll = function (elem, leaveFor) {
      var scrollOptions = 'scrollIntoSectionOptions';
      if (leaveFor) {
      ...
      if (_sections[sectionId][scrollOptions] !== undefined && _sections[sectionId][scrollOptions] !== "" &&  _sections[sectionId][scrollOptions] !== null) {
        ...
        if (_sections[sectionId][scrollOptions] !== 'no-scroll') {
    ...
    if (selector.charAt(0) == '@') {
      ...
      if (next) {
        ...
        if (isNavigable(next, nextSectionId)) {
          ...
          return focusElement(next, nextSectionId, direction, true);
        }
      }
    }
    return false;
  ...
  function focusSection(sectionId) {
    ...
    for (var i = 0; i < range.length; i++) {
      ...
      if (next) {
        console.log('focus element 3');
        return focusElement(next, id, undefined, true);
      }
    }
